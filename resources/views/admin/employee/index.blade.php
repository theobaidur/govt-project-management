                    ...
                    <div class="card-body" v-cloak>
                        <form class="d-print-none" @submit.prevent="">
                            <div class="row justify-content-md-between">
                                ...
                                <div class="col col-md-3 form-group">
                                    <multiselect v-model="selectedEmployeeDesignations"
                                        :options="{{ $designations->toJson() }}"
                                        label="name"
                                        track-by="key"
                                        placeholder="{{ trans('admin.employee-designation.title') }}"
                                        :limit="2"
                                        :multiple="true">
                                    </multiselect>
                                </div>
                        ...
                        <table class="table table-hover table-listing">
                            <thead>
                                <tr>
                                    ...
                                    <th is='sortable' :column="'email'">{{ trans('admin.employee.columns.email') }}</th>
                                    <th is='sortable' :column="'phone'">{{ trans('admin.employee.columns.phone') }}</th>
                                    <th is='sortable' :column="'department_id'">{{ trans('admin.employee.columns.department_id') }}</th>
                                    <th is='sortable' :column="'employee_designation_id'">{{ trans('admin.employee.columns.employee_designation_id') }}</th>
                                ...
                                <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="8">
                                        <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/employees')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('brackets/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                                    href="#" class="text-primary" @click="onBulkItemsClickedAllUncheck()">{{ trans('brackets/admin-ui::admin.listing.uncheck_all_items') }}</a>  </span>

                                        <span class="pull-right pr-2">
                                            <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/admin/employees/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                        </span>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                    ...
                                    <td>@{{ item | valueOrDefault('employee_designation', 'name', item.employee_designation_id) }}</td>
